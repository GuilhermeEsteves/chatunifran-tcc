...
.directive('gear', function(){
	return {
		...
		controller: function($scope, $element, $attrs){
			$scope.$parent.gear = {
				author: 'Eng Solutions',
				email: 'eng@hotmail.com',
				version: '0.1'
			};
		}
	};
...
.directive('gearDialog', function(){
	return {
		...
		replace: true,
		transclude: true,
		template: '<div class="gear-dialog-wrap" ng-class="{\'gear-open\': isOpened, \'gear-focus\': isFocused}" ng-click="close($event)"><div class="gear-mt gear-dialog" ng-style="{\'width\': gearWidth}"><div class="gear-dialog-title" ng-if="gearTitle">{{gearTitle}}</div><div class="gear-dialog-content" ng-transclude></div></div></div>',
		scope: {
			'id': '@',
			'gearWidth': '@',
			'gearTitle': '@'
...
.directive('gearTooltip', function(){
	return {
		...
		link: function(scope, element, attrs, ctrl) {
			var config = {
				content: scope.gearTooltip,
				color: scope.gearColor || 'blue',
				background: scope.gearBackground || 'black'
			};
			if (scope.gearWidth)
...
.directive('gearColorPickerGroup', function($timeout){
	return {
		...
		templateUrl: 'content/lib/gear/template/color-picker-group.html',
...
.directive('gearDatePicker', function($timeout){
	return {
		...
		templateUrl: 'content/lib/gear/template/date-picker.html',
		link: function(scope, element, attrs, ctrl) {
...
.directive('gearDatePickerGroup', function($filter, $timeout){
	return {
		...
		templateUrl: 'content/lib/gear/template/date-picker-group.html',
		replace: true,
		link: function(scope, element, attrs, ctrls) {
			'autocomplete' in attrs && element.find('input:first').attr('autocomplete', attrs.autocomplete);
			scope.required = 'required' in attrs || scope.gearRequired;
			scope.$watch('gearRequired', function(value){
				if (value !== undefined)
...
.directive('gearUserArea', function($timeout){
	return {
		...
		templateUrl: 'content/lib/gear/template/user-area.html',
		link: function(scope, element, attrs){
			element.bind('click', function(){
...
.directive('gearSnackContainer', function(gearSnack, gearColor, $timeout){
	return {
		restrict: 'E',
		templateUrl: 'content/lib/gear/template/snack-container.html',
		link: function(scope, element, attrs) {
			var timeout;
			scope.bars = [];
			gearSnack.newAdded(function(bar){
				...
				bar = angular.extend({}, gearSnack.getDefaults(), bar);
				scope.bars.push(bar);
...
.directive('gearSnackBar', function(gearSnack){
	return {
		restrict: 'E',
		templateUrl: 'content/lib/gear/template/snack-bar.html',
		scope: {
			'gearBar': '='
		},
		link: function(scope, element, attrs) {
...
.directive('gearTimePicker', function($timeout){
	return {
		...
		templateUrl: 'content/lib/gear/template/time-picker.html',
		link: function(scope, element, attrs, ctrl) {
			scope.hourType = 24;
...
.directive('gearTimePickerGroup', function($filter, $timeout){
	return {
		...
		templateUrl: 'content/lib/gear/template/time-picker-group.html',
		replace: true,
		link: function(scope, element, attrs, ctrls) {
			'autocomplete' in attrs && element.find('input:first').attr('autocomplete', attrs.autocomplete);
			scope.required = 'required' in attrs;
...
.directive('gearMainMenu', function(){
	return {
		...
		templateUrl: 'content/lib/gear/template/main-menu.html',
...
.directive('gearMainMenuList', function(){
	return {
		...
		templateUrl: 'content/lib/gear/template/main-menu-list.html',
		link: function(scope, element, attrs) {
			scope.filteredList = [];
...
.directive('gearMainMenuItem', function($compile){
	return {
		...
		templateUrl: 'content/lib/gear/template/main-menu-item.html',
		link: function(scope, element, attrs, ctrl) {
